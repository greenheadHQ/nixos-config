...
> ⚠️ **Historical record**: 이 문서는 과거 시행착오의 기록입니다. 현재 구성과 불일치할 수 있으며, 최신 정보는 각 SKILL.md와 모듈별 문서를 참조하세요.
...
다음 앱들이 대안으로 언급되지만, **실제 테스트하지 않았으므로 동일한 문제가 발생할 수 있음**:

| 앱 | 가격 | 특징 | 검증 여부 |
|----|------|------|-----------|
| **Blink Shell** | $19.99 | Chromium HTerm 기반, Mosh 네이티브 지원 | ❓ 미검증 |
| **Prompt 3** | $19.99 | Panic 개발 | ❓ 미검증 |
| **ServerCat** | 무료/Pro | 서버 모니터링 + SSH | ❓ 미검증 |

> **주의**: 인터넷에서 "인코딩 처리 우수"라고 언급되더라도, 한국어 IME 문제는 별개일 수 있음.
> 유료 앱 구매 전 무료 체험판이나 리뷰를 확인하거나, 직접 한국어 입력 테스트 후 결정 권장.
...
- **의도**: rip만 제거
- **결과**: 4개 inputs 전부 업데이트 (nixpkgs, home-manager, nix-darwin, nix-vscode-extensions — 당시 VSCode 시점. issue #329 / PR #330 마이그레이션으로 Zed 전환 시 일시 제거됐다가 그 reverse PR로 다시 활성: 현재 에디터 모듈은 `modules/darwin/programs/vscode/`)
...
```bash
nfu           # fzf multi-select로 업데이트할 input 선택
nfu -a        # 모든 input 일괄 업데이트
nfu --cores N # nrs에 --cores N 전달 (NixOS 과열 방지)
```

nfu는 update → FOD hash fix → nrs를 원자적으로 실행하며, 실패 시 자동 롤백한다.
...
>
> **Note**: 이 시행착오의 대상 프로그램(Cursor)은 issue #171 / PR #181에서 VSCode로 대체된 뒤 issue #329 / PR #330에서 Zed로 마이그레이션됐고, 다시 그 reverse PR로 VSCode가 복귀했다. 현재 에디터 모듈은 `modules/darwin/programs/vscode/`.
