...
이 follow-up의 `닫힘` 상태는 Phase 2a Post-Merge Remediation이 canonical이다. 최소 조건은 MiniPC OpenSSH `KbdInteractiveAuthentication = false` 적용, `sudo -n sshd -T`의 `kbdinteractiveauthentication no` 검증, **iPhone·iPad가 `mobile-ssh` 공유 키로 접속해 server-side accepted fingerprint == `mobile-ssh` 키(`SHA256:hlE5…`)임이 로그로 확인**, 같은 device IP + user + attempt window의 keyboard-interactive/PAM 실패 로그 부재, Required Before iPhone Remediation 결정 완료, Phase 2a/master PRD 기록 완료다. (iPhone/iPad는 Termius 계정 동기화로 단일 `mobile-ssh` 공유 키를 쓰므로 디바이스별 개별 fingerprint match가 아니라 양 기기의 mobile 키 접속 확인이 기준이다.) Policy Follow-Up 질문은 immediate recovery `닫힘`을 막지 않는다. Fallback Only decision set은 fallback을 실제로 사용하기로 선택한 경우에만 `닫힘` 조건에 포함한다.

### Fingerprint Inventory Table

Measured 2026-05-30~31 (MiniPC `sshd` journal 보존 시작 2026-05-02). 2026-05-31 iPhone/iPad는 Termius 동기화로 단일 `mobile-ssh` 공유 키로 통합됐다(직전 rotation한 iphone-ssh 키를 mobile로 재명명, ipad-ssh는 폐기). 마지막 row는 authorized_keys에 더 이상 없는 retired 공유 키로 journal `Accepted publickey` 로그에서만 관측된다.

| Key label | SHA256 fingerprint | Source command |
|---|---|---|
| `mobile-ssh` (iPhone·iPad 공유) | `SHA256:hlE5JoF+9xFVJmw3BVN/+NC5134uDU5sv5KfdbNmq1k` | 2026-05-31 mobile 공유 키로 통합(직전 iphone-ssh 키 재명명). 폐기: 직전 iphone-ssh `qkDV…`, ipad-ssh `rtx6…`/`rCm2…`. 배포 후 `ssh-keygen -lf /etc/ssh/authorized_keys.d/greenhead`로 재확인 |
...
| iPhone **(closeout 2026-05-31)** | `greenhead` | `100.76.27.1` | 2026-05-31T00:14:41 KST | `mobile-ssh` | `SHA256:hlE5…q1k` | `SHA256:hlE5JoF+9xFVJmw3BVN/+NC5134uDU5sv5KfdbNmq1k` | `2026-05-31T00:14:41 Accepted publickey … SHA256:hlE5…` | match (rejected 없음) | 없음 (`passwordauthentication no`·`kbdinteractiveauthentication no`) | **PASS** |
| iPad **(closeout 2026-05-31)** | `greenhead` | `100.114.211.7` | 2026-05-31T00:15:08 KST | `mobile-ssh` | `SHA256:hlE5…q1k` | `SHA256:hlE5JoF+9xFVJmw3BVN/+NC5134uDU5sv5KfdbNmq1k` | `2026-05-31T00:15:08 Accepted publickey … SHA256:hlE5…` | match (rejected 없음) | 없음 | **PASS** |
...
- 2026-05-31: mobile 공유 키 통합으로 follow-up 사실상 해소. Termius 계정 동기화가 host의 key 지정과 표준 키 private 본체를 기기 간 복제해 iPhone/iPad 디바이스별 격리가 구조적으로 불가함을 웹조사 + MiniPC journal 실측으로 확정(SSH ID passkey는 device-bound로 격리 가능하고 MiniPC OpenSSH 10.3p1이 `sk-ssh-ed25519@openssh.com` 지원하나, 설정 복잡도로 보류). iphone/ipad 분리 키를 단일 `mobile-ssh`(직전 iphone-ssh 키 `SHA256:hlE5…` 재명명)로 통합, ipad-ssh(`SHA256:rtx6…`) 폐기, MiniPC authorizedKeys를 3개(`mac-ssh`/`mobile-ssh`/`emergency`)로 축소. master PRD SC-4·FR-10·Scenario 4·5·A-2·Risks 동반 개정. closeout 기준: iPhone·iPad가 mobile 키로 접속해 server-side accepted fingerprint == `hlE5…` 확인 + PAM 실패 부재.
- 2026-05-31: **closeout 완료**. mobile 공유 키 배포(#869) 후 iPhone(`100.76.27.1`, 00:14:41)·iPad(`100.114.211.7`, 00:15:08)가 모두 `mobile-ssh`(`SHA256:hlE5…`)로 `Accepted publickey` 확인, 같은 IP에 keyboard-interactive/PAM 실패 부재. Closed Status Definition 전 조건 충족 → 추적 이슈 #866 close. (접속 직전 `[preauth] Disconnected`는 Termius의 다중 키 순차 offer 협상이며 PAM 실패 아님.)
