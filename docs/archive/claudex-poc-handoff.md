...
- **현재 상태**: 배포 완료. 두 Darwin 호스트와 NixOS 호스트(`greenhead-minipc`)를 허용하는 descriptor schema 3, 통합 CLI + on-demand lifecycle(#1192), private traffic gate, launchd/systemd-user 실패 복구, graceful drain, credential 손상 복구가 가동 중이며 proxy pin은 `7.2.111`(#1199)이다. `nrs`·headless completion·`--fast`·auto-compact A/B 실측은 §5에 기록돼 있다.
- **다음 액션**: 새 머신 온보딩은 §7~§11(Phase 1~5)을 순서대로 수행한다. 잔여 추적 항목은 `--fast` 티어의 live 대조(#1112)와 Codex catalog window 재상향(#1113), 모델 pin 재조정 트리거(#1130)다.
- **Blockers**: 없다. proxy pin을 바꿀 때는 §13 체크리스트를, 모델 pin을 바꿀 때는 #1130 체크리스트(§3·§4의 문서 값과 `tests/eval-tests.nix`·`tests/suites/claudex.sh`의 리터럴을 같은 커밋에서 갱신)를 따르고, 실제 OAuth·refresh·process/network 관측은 실행 직전 사용자 승인을 받는다.
...
- `modules/shared/programs/claudex/files/claudex.sh`
  ...
  - model 계약은 역할별로 고정한다: default 모드 main = `gpt-5.6-sol`, 모든 모드의 subagent(`CLAUDE_CODE_SUBAGENT_MODEL`) = `gpt-5.6-sol`, `--mixed` 모드 main = `claude-opus-5`. 사용자 `--model` override는 여전히 거부된다. 이 고정은 **세션 시작값**이다 — 세션 중 `/model` 전환(CLI 기능)으로 카탈로그의 다른 모델로 바꿀 수 있음이 실측됐다(2026-07-17). 구독 플랜의 모델 정책 변화 시 pin 재조정은 #1130이 추적한다. effort는 기본 `high`이며, 명시적 `claudex --effort <low|medium|high|xhigh|max|ultra>` 인자만 세션 값을 바꾼다. `ultra`는 wrapper가 실행하는 CLI가 argv에서 warn-then-ignore하므로 env 값으로만 전달한다.
...
| 모델 (mixed main) | `claude-opus-5` (`claudex --mixed`; descriptor `.model`은 default main alias; 재조정 트리거 #1130) |
...
| Claude 실행 파일 | `$HOME/.local/bin/claude` — **claudex가 핀하지 않는다.** 사용자 자신의 설치본이며 `modules/shared/programs/claude/files/settings.json`의 `autoUpdatesChannel: "latest"`로 자동 갱신된다. 아래 CLI 동작 서술의 `2.1.210 실측`은 그 시점 관측이지 이 저장소가 고정한 계약이 아니다 (재검증: `"$HOME/.local/bin/claude" --version`) |
...
14. replacement는 기존 canonical 경로를 같은 filesystem의 검증된 staged 파일로 원자 교체한다. v7.2.73 소스 확정 당시 기준 — CLIProxyAPI는 credential 파일명을 생성할 때 계정 정보를 반영하지만 loader는 auth-dir의 JSON 내용을 파싱하고 watcher identity는 경로를 기준으로 하므로, 기존 canonical 경로를 유지해 실행 중 watcher가 provider 삭제/추가로 오인하지 않게 한다. (이후 upstream이 filename 생성 v7.2.89, filestore equality v7.2.102, watcher deletion guard v7.2.103을 변경했다 — 현재 pin에서는 소스 재검증 전이며, pin 갱신 시 아래 §13 체크리스트의 source watchlist 항목으로 재확인한다.)
...
|---|---|---|
| main model (`--model`) | `gpt-5.6-sol` | `claude-opus-5` |
...
- **subagent effort 세밀 제어 불가 (기능 한계)**: wrapper는 `CLAUDE_CODE_SUBAGENT_MODEL`로 subagent 모델만 고정 모델에 맞추고, effort는 세션 단위 값 하나(기본 `high`, `claudex --effort`로 조정 가능)를 세션 전체가 공유한다. subagent effort만 별도로 낮추는 수단은 여전히 없다(wrapper가 실행하는 Claude Code CLI 자체의 제약). 대량 fan-out 워크플로우에서 토큰 소모가 부담이면 세션 effort 자체를 낮춰서 실행한다.
- **context 사용률 표시는 근사치 (기능 한계)**: pinned proxy는 SSE `message_start`에 usage `{input_tokens:0, output_tokens:0}`을 하드코딩한다 (Codex Responses API가 스트림 시작 시점에 usage를 주지 않기 때문; 실제 값은 `message_delta`에만 실림). upstream은 동일 보고(router-for-me/CLIProxyAPI#1700)를 수정 거부로 닫았고 v7.2.77까지 이 변환기는 변경이 없다. wrapper가 실행하는 CLI(2.1.210 실측)는 각 assistant 메시지에 `message_start` 스냅샷을 박제하고 "usage 합>0인 마지막 assistant 메시지"를 컨텍스트 추적 anchor로 삼으므로, 이 경로에서는 anchor가 없어 **문자수 기반 로컬 추정으로 fallback**한다(과대 경향). 여기에 미인식 모델의 200k 기본 가정이 겹치면 statusline이 조기에 "100% context used"로 포화한다. wrapper-owned `CLAUDE_CODE_MAX_CONTEXT_TOKENS=272000`은 분모를 공식 Codex catalog의 raw `context_window`로 교정하는 완화이며, 분자(로컬 추정)의 오차는 남으므로 **표시되는 %는 근사치다**. Codex 자체는 raw `272000`에 `effective_context_window_percent=95`를 적용해 `258400`을 usable window로 노출하지만, Claude Code는 별도로 output/compact headroom을 적용하므로 이 effective 값을 raw window 자리에 다시 넣지 않는다. 과거 `258000`은 2026-07-15의 Codex UI effective 표시를 반올림해 옮긴 값이라 headroom을 이중 적용했다. 공식 변경 근거는 [openai/codex#33961](https://github.com/openai/codex/pull/33961)이며, 현재값은 `codex debug models | jq '.models[] | select(.visibility == "list") | {slug,context_window,max_context_window,effective_context_window_percent}'`로 재검증한다 — 모델 slug를 박은 `select`는 pin이 바뀌면 에러 없이 빈 출력을 내 "확인했는데 아무것도 없다"로 오독되므로, 전 모델을 나열해 pin 대상 행을 직접 고른다. 출력의 `max_context_window`(2026-09 기준 872000)는 추종 대상이 아니다 — wrapper가 맞추는 값은 raw `context_window`다. [#1113](https://github.com/greenheadHQ/nixos-config/issues/1113)은 Codex catalog가 향후 `372000`으로 재상향되는 시점을 계속 추적하며, 이번 raw/effective 계층 교정으로 닫지 않는다.
- **auto-compact는 별도 env 채널이 필요 (기능 한계 → 해소)**: `CLAUDE_CODE_MAX_CONTEXT_TOKENS`는 미인식 모델의 로컬 window·사용률 분모·사전 요청 gate를 바꾸지만 compact window의 **source**는 바꾸지 않는다. wrapper가 실행하는 CLI(2.1.210 실측)의 auto-compact 메인 경로는 source가 "auto"면(로컬 세션 가드) compact를 비활성화한다. 미인식 모델은 내장 window 테이블에 없어 명시 채널 없이는 source가 항상 "auto"이므로, claudex 세션은 auto-compact가 구조적으로 꺼져 있었다 — statusline이 "N% until auto-compact" 대신 "N% context used"로 표시되는 것이 가시적 증상이다. wrapper가 `CLAUDE_CODE_AUTO_COMPACT_WINDOW`(공식 env 채널, 유효 100k–1M)를 같은 raw 값으로 재발행해 source를 "env"로 전환하고 임계 검사(≈ window − 출력예약 − 13k)를 활성화한다. 이 자체 reserve 때문에 raw 272k를 선언해도 compaction은 Codex의 258.4k usable window보다 먼저 시작한다. 참고로 한계 초과 시 에러 기반 회복도 기대할 수 없다: pinned proxy는 upstream 문구를 그대로 전달할 뿐 Anthropic 정식 문구로 재작성하지 않는다. 활성화는 A/B로 실측 완료(2026-07-15): `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=1` + 2턴 stream-json 멀티턴에서 새 wrapper는 `status:"compacting"` 이벤트를 발동(미니 대화라 `too_few_groups`로 요약은 실패 — 트리거 사슬 동작 증거로 충분), 동일 조건에서 `AUTO_COMPACT_WINDOW`만 뺀 대조군은 compact 신호가 전무했다. 대화형 세션의 가시 신호는 statusline "% until auto-compact" 표기 전환이다.
...
- **fast 티어의 조용한 폴백과 관찰 한계 (기능 한계)**: `claudex --fast`는 request body에 `service_tier:"priority"`를 주입하지만, Codex OAuth 계정에 fast 자격이 없으면 백엔드가 에러 없이 default 티어로 처리한다(upstream 실측 사례: openai/codex#14204). wrapper는 `exec` 구조라 응답을 검사할 수 없으므로 세션 내 자동 감지는 없다. 응답 쪽 관찰도 제한적이다 — v7.2.73에서는 proxy가 Anthropic 형식으로 역변환한 응답 `usage.service_tier`에 `--fast` 유무와 무관하게 고정 `"standard"`를 넣는 것이 관측됐다(2026-07-15 실측, fast/plain 대조 동일). 그 실측 기준으로는 **응답 usage로 백엔드 티어 반영을 판별할 수 없다**. 현재 pin v7.2.111의 응답 동작은 live 대조 전이다 — 소스 조사에서 응답 방향 translator의 service_tier 수정 커밋은 확인되지 않았으나(usage 기록의 ResponseServiceTier 분리 v7.2.74, 요청 방향 speed=fast 처리 v7.2.85만 존재), 배포 후 fast/plain 대조(#1112)로 확정한다. 검증 가능한 계약은 요청 주입까지다: loopback 캡처 서버에 fast settings variant로 headless 요청을 보내면 body에 top-level `"service_tier":"priority"`가 실리고, 기본 variant에서는 필드 자체가 없다(동일 실측). 백엔드 전달은 pinned proxy 소스의 claude→codex 변환기가 `service_tier`(fast/priority → `priority` 정규화, 그 외 drop)를 Codex 요청에 주입하고 executor가 이 필드를 삭제하지 않는 것으로 보증한다. 실제 1.5x 처리 여부는 계정 자격과 백엔드 정책에 종속되며 이 경계 밖이다.
- **bypassPermissions 기본 시작 (의도된 트레이드오프)**: wrapper는 `--dangerously-skip-permissions`로 Claude Code를 실행해 모든 세션이 bypass 모드로 시작한다. wrapper가 실행하는 CLI가 시작 플래그 없이는 세션 중 bypass 전환을 허용하지 않아 사용자가 기본 bypass를 선택했다. 이를 실제로 동작시키기 위해 `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB`도 `0`으로 opt-out했다 — `1`이면 wrapper가 실행하는 CLI의 allowed_non_write_users hardening이 permission mode를 default로 강제해 bypass 플래그를 조용히 무력화한다(2.1.210 실측). 잔여 노출은 세션 내 서브프로세스가 wrapper-owned loopback API key 환경값을 볼 수 있다는 것인데, 이 key는 `127.0.0.1:8317` 전용이라 영향이 국소적이다. 권한 프롬프트가 필요한 환경에서는 wrapper의 bypass 플래그를 제거해야 하며, 그 경우 세션 중 bypass 전환 옵션도 함께 사라진다.
...
## 13. proxy pin 갱신 체크리스트

CLIProxyAPI pin(`modules/shared/programs/claudex/cli-proxy-api-pin.json`)을 올릴 때마다 이 순서로 검증한다. 각 항목은 v7.2.111 갱신에서 실제 수행한 절차의 기록이기도 하다.

1. 해시 검증: upstream release `checksums.txt`를 받아 darwin arm64 / linux amd64 asset의 SHA-256을 대조하고, `nix hash convert --hash-algo sha256 <hex>`로 SRI 변환. darwin은 `nix store prefetch-file <asset URL>`로 실측 해시 일치까지 확인한다.
2. pin 소비자 동기화: `rg -n '<이전 버전>'`으로 잔존 참조를 전수 확인 — `tests/eval-tests.nix`(D16/D18/D18b)와 `tests/fixtures/claudex-home.nix`는 리터럴 동기화 필수, 문서·CIR의 실측 역사 기록은 시제를 고정("v<N> 소스 확정 당시")하고 현재-상태 서술만 갱신한다.
3. 릴리스 노트 전수 검토: 이전 pin → 새 pin 구간에서 breaking config 변경 여부와 source watchlist(credential filename·filestore·watcher, `service_tier`/usage, config 로더) 관련 커밋을 추린다. 관련 커밋이 있으면 §5/§12의 해당 서술에 "재검증 전" 표시를 남긴다.
4. 정적 검증: eval 테스트(IFD/no-IFD)와 shell 테스트를 통과시킨다.
5. ad-hoc 기동: 새 바이너리를 빈 auth-dir + config-template 렌더로 임시 포트에서 기동해 config 파싱·watcher 시작을 확인한다 (실제 credential은 사용 금지 — refresh token rotation으로 원본이 무효화될 수 있다).
6. 배포 후 live 대조 (#1112): `nrs` 배포 뒤 fast/plain completion의 응답 `usage.service_tier`를 대조하고 §4의 관찰 한계 서술을 갱신한다.

## 14. 현재 다음 행동 요약

배포는 이미 끝났고(§5), 아래는 잔여 행동이다.

1. 새 머신 온보딩: §7~§11을 순서대로 수행한다.
2. pin(모델·proxy 버전)을 바꾼 경우: 정적·fixture 검증을 통과시킨 뒤 승인된 호스트에서 `nrs` → `claudex status` → session 자동 시작 → graceful stop을 실측하고, 모델 pin이면 `claudex --mixed -p "Reply with exactly your model name"`으로 자가 보고까지 확인한다(#1130).
3. 잔여 추적: `--fast` 티어 live 대조(#1112), Codex catalog window 재상향(#1113), 별도 승인을 받은 경우에만 #1108의 network/refresh 관측.
