...
- `modules/shared/programs/claudex/files/claudex.sh`
  ...
  - context window를 wrapper-owned `CLAUDE_CODE_MAX_CONTEXT_TOKENS=258000`으로 고정한다 (pinned CLI의 비-claude 모델 전용 공식 override; 미인식 모델의 200k 기본 가정을 Codex 앱이 현재 보고하는 실효 한계 258k로 교정). 상속 환경값은 scrub된다. 이 값은 upstream의 임시 하향분이라 재상향 시 재조정이 필요하다 — 아래 "context 사용률 표시는 근사치" 항목과 후속 이슈 참조.
...
| Context window override | `CLAUDE_CODE_MAX_CONTEXT_TOKENS=258000` (wrapper-owned; upstream 임시 하향 추종) |
