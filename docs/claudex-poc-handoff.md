...
- **대형 번들 스킬은 이 window에서 단독으로 한계를 유발 (용량 한계, 회피 전용)**: Claude Code 내장(번들) 스킬은 로드 시 본문 전체가 단일 레코드로 컨텍스트에 주입되는데, `claude-api`는 그 크기가 약 190k 토큰이라 258k window의 약 4분의 3을 단독으로 차지한다 (2026-07-18 실측: 로드 전 약 27k → 로드 후 `219.1k/258k`(85%)에 `Free space` 5.9k; 세션 로그의 스킬 주입 레코드 단일 크기 812,741 bytes로 로그 전체의 97%). 대화가 1턴뿐이어도 스킬 로드만으로 `Context limit reached`가 발생하며, **auto-compact는 이 경우 구제하지 못한다** — 이 차단은 요청 전송 전 컨텍스트 조립 단계의 사전 검사인 반면 auto-compact는 응답 수신 후 임계를 판정하는 사후 메커니즘이라 개입 차례가 오지 않는다(같은 세션 로그에 `assistant` 레코드 0건, compact 계열 이벤트 0건 — 위 항목의 트리거 사슬이 정상이어도 이 경로에는 닿지 않는다). 설령 발동해도 compact가 줄이는 대상은 과거 대화인데 실제 대화량은 8k 수준이고 공간을 먹는 주체는 방금 로드한 스킬 본문이라 효과가 없다. 원인 축은 스킬의 출처가 아니라 **크기 대 window**다: 같은 절차에서 이 저장소의 로컬 스킬(`/playwright-cli` 등)은 Messages 5~8k로 무해하고, 순수 Claude 세션(Opus 4.8, 1M window)은 동일 스킬에서 약 20%만 차지해 정상 동작한다. 회피 수단은 세 가지다 — 해당 스킬을 트리거하지 않기, `claudex --mixed`(메인이 인식 모델이라 window가 큼)로 열기, 이미 걸렸다면 `/clear`. window(258k)는 백엔드 실효 한계 추종값이고 스킬 크기는 upstream 번들이라 양쪽 모두 이 저장소가 통제할 수 없으므로 코드 수정 대상이 아니다([#1140](https://github.com/greenheadHQ/nixos-config/issues/1140)에 재현 절차와 4조건 대조표 기록). 재검증 명령: 세션에서 `/claude-api`를 직접 호출한 뒤 `/context`의 상단 게이지와 `Free space`를 확인한다 — 자동 트리거는 로드가 중단되어 상단 게이지(11%)와 카테고리 표시(`Messages` 199.6k)가 어긋나므로 슬래시 직접 호출이 정확한 계측 경로다. window가 재상향되면([#1113](https://github.com/greenheadHQ/nixos-config/issues/1113)) 같은 스킬이 약 51%로 내려가 단독으로는 한계를 넘지 않으므로, 그 시점에 이 항목도 함께 재확인한다.
