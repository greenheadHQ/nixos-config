...
- `--bare`, `--agent`, `--agents`가 고정 모델 계약을 우회하는지
- interactive `/model`은 시작 모델 계약을 우회한다. default→Claude에서는 전역 `AUTO_COMPACT_WINDOW=272000`이 남아 약 239k에서 조기 compact하고, mixed→GPT에서는 AUTO가 없어 auto-compact가 비활성이다. 다른 non-Claude 모델도 `MAX_CONTEXT_TOKENS=272000`을 상속한다. Stage 2에서는 전환 금지 또는 모델별 env 재계산을 설계한다.
