...
# secrets/secrets.nix에서도 import하므로 SSH 키의 유일한 정의 위치
{
  ...
  ankiMcp = {
    ...
    # 메시지 구분에 쓰는 traceparent는 OpenAI가 보장한 값이 아니다. 헤더가 없으면 "확인 필수"로 물러나고,
    # 같은 메시지 판정 때문에 확인이 모두 막히면 enable=false로 끈다 (anki-host README 참고).
    writeConfirmGate = {
      enable = true;
  ...
  macos = {
    ...
    paths = {
      ...
      vscodeAppRelative = "Applications/Home Manager Apps/Visual Studio Code.app";
