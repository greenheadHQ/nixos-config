...
미리보기 대상이 아닌 20건 이하 변경(추가·필드 수정·태그·미디어 저장 등)은 그런 쓰기도 바로 반영된다.
...
이 안내는 ChatGPT 앱 설명(설정 > 플러그인 > Anki > ⋯ > 설명 편집)에만 둔다. 앱 설명은 모델 컨텍스트에서
서버 지침보다 앞에 붙고 ChatGPT에서만 쓰인다. 이 서버는 Claude 앱·Desktop 등 다른 클라이언트도 함께 쓰므로,
ChatGPT의 병렬 후보에만 해당하는 안내는 공통 도구 설명이나 서버 지침에 넣지 않는다.
저장소 밖 설정이라 현재 값을 아래에 기록한다. 배포 뒤 앱 정보의 "새로 고침"을 실행해도 이 값은 바뀌지 않았다.
...
변경 전 7/10이었다. `anki_recent_operations` 도구 설명에 같은 안내를 한 문장 넣었을 때 3/10(부분 단정 2 별도),
앱 설명까지 더했을 때 0/17이었다. 재시도 전 확인과 같은 `request_id`를 안내한 응답은 앱 설명을 더한 뒤에만 나왔다.
도구 설명의 문장은 효과가 작고 다른 클라이언트에 해당하지 않아 뺐다. 앱 설명만 둔 상태는 따로 측정하지 않았다.

보이지 않는 후보의 쓰기를 서버에서 막는 방법도 시험했지만 채택하지 않았다([#1413](https://github.com/greenheadHQ/nixos-config/pull/1413)).
서버는 요청한 모델을 알 수 없어 모든 쓰기에 확인 단계가 붙었고, 확인 때 긴 데이터를 다시 보내야 하는 결함이 생겼다.
...
사용자 기기에 있는 이미지는 MCP Apps 업로드 위젯으로 원본 그대로 저장한다.
...
- **흐름**
  ...
  - 사용자가 이미지를 한 장 이상 고르면 위젯이 한 장씩 차례로 앱 전용 도구 `anki_upload_ticket`
    (`_meta.ui.visibility: ["app"]`)으로 1회용 입장권을 받아 `POST /upload?ticket=…`로 파일을 보낸다.
  - 서버는 `store_media`와 같은 원장 경로로 한 장씩 즉시 저장한다. 위젯은 모든 파일을 처리한 뒤 결과를
    `ui/update-model-context`와 `ui/message`로 각각 많아야 한 번 알리고, 모델은 필드에 `<img src="파일명">`을 넣는다.
    - 메시지는 저장된 파일(이미 있던 이미지 포함)이 하나라도 있을 때만 보낸다. 여러 장이면 저장·이미 있음·실패·
      저장 여부 미확인(무응답 등)을 모두 적는다.
    - 컨텍스트는 저장 결과나 무응답이 하나라도 있을 때만 보낸다. 모두 형식·크기 등으로 거절되면 둘 다 보내지 않고,
      사용자는 위젯에서 사유를 본다.
  - 채팅에 첨부했던 이미지는 위젯에서 다시 고른다.
  - 모델은 위젯으로 올린 이미지를 볼 수 없다. 내용이 필요하면 같은 이미지를 채팅에 첨부한다. 위젯의 안내 문구(사용자용)와
    도구 결과 문장(모델용)이 이를 알린다.
- **한도·형식**
...
- **클라이언트 동작** (2026-09-24·25 lab 실측)
  - ChatGPT(웹·iPhone)와 Claude(웹·iPhone) 모두 위젯을 렌더링하고, 원본을 바이트 그대로 전달했다.
    여러 장 선택도 웹과 iPhone 사진 선택기 모두에서 된다. iPhone 카메라 사진은 여러 장을 골라도 원본 해상도로 왔다(ChatGPT·Claude iPhone, 두 앱이 같은 바이트).
  - Claude는 `ui/message`를 입력창에 채우기만 하고 사용자가 전송한다. ChatGPT는 바로 전송했으나(2026-09-25 새벽),
    같은 날 오후부터는 메시지마다 "후속 프롬프트를 보내시겠어요?" 확인 창을 띄워 사용자가 보내기를 눌러야 전송했다.
  - ChatGPT는 `ui/update-model-context`의 텍스트를 모델에 전달하지만, claude.ai는 전달하지 않는다(텍스트만 보내도 같다).
    그래서 메시지를 보낼 때는 모델이 알아야 할 내용(실패 포함)을 메시지에 적는다.
  - 두 클라이언트 모두 `ui/message`·`ui/update-model-context`에 넣은 이미지는 모델에 넘기지 않는다. 호스트는 그래도
    성공(`{}`)으로 응답하고, ChatGPT 모델은 받지 못한 이미지의 내용을 지어냈다.
  - 채팅에 이미지를 첨부하고 카드를 요청하면 두 클라이언트 모두 스스로 위젯을 열고 같은 파일을 다시 고르라고 안내한다.
    ChatGPT는 한 요청에 위젯을 두 번 연 적이 있어, 도구 설명에 요청마다 창을 하나만 연다고 적는다.
    이렇게 바꾼 뒤(2026-09-25 오후)에는 첨부 흐름에서 ChatGPT 10회·Claude 4회 모두 위젯을 하나만 열었다.
  - claude.ai는 "Anki" 없이 "업로드 창 열어줘"라고만 하면 채팅 첨부 방법을 안내하기도 했다(3회 중 2회).
    "Anki"를 넣은 요청은 첨부 흐름을 포함해 6회 모두 위젯을 열었다.
  ...
  - 두 클라이언트 모두 도구 목록과 위젯 HTML을 캐시한다. 도구나 위젯을 바꿔 배포하면 ChatGPT는 앱 설정 `앱 관리`의 **도구 새로 고침**,
...
- **설계 근거**: 아래 선택을 바꾸기 전에 그 이유가 아직 성립하는지 확인한다.
  - **채팅 첨부(ChatGPT `openai/fileParams`)를 쓰지 않는다.**
    - ChatGPT 전용이고, 이미 줄인 사본(웹 긴 변 2048px, iPhone 1536px)만 준다.
    - 서버가 약 5분 뒤 만료되는 외부 URL을 가져와야 한다. 이 서버는 요청을 받아 다른 호스트로 접속하는 경로를 만들지 않으므로 URL을 가져오지 않는다.
    - 위젯은 두 클라이언트 모두에서 브라우저가 원본을 직접 보낸다.
  - **파일을 고르면 바로 저장한다.**
    - 사용자의 파일 선택이 곧 승인이다.
    - 임시 보관한 뒤 모델이 확정하는 방식은 보관소·만료 정리·확정 도구가 더 필요하고, 확정이 다시 모델의 쓰기가 된다.
    - 쓰이지 않은 파일은 Anki "미디어 확인"으로 정리한다.
  - **5 MiB 한도를 유지하고, 넘는 파일만 줄인다.**
    - 한도를 올리면 AnkiWeb 동기화 용량과 휴대폰 Anki의 부담이 커진다. 넘는 파일을 거절하면 사용자가 직접 줄여야 한다.
    - 한도 안의 파일은 다시 인코딩하지 않아 원본 그대로 남는다.
    - GIF는 캔버스로 줄이면 움직임이 사라지므로 거절한다.
  - **입장권은 업로드 직전에 앱 전용 도구로 받는다.**
    - 위젯은 호스트의 샌드박스 iframe에서 돌아 OAuth 토큰을 쓸 수 없다.
    - 위젯이 처음 뜰 때 미리 넘기면 사진을 고를 때까지 유효해야 해 수명이 길어진다. 또 도구 결과가 모델에게 보이지 않는다는 보장이 클라이언트마다 없다.
    - 위젯이 뜨는 도메인은 호스트마다 달라 도메인 허용 목록 대신 입장권으로 막는다. 속도 제한은 두지 않는다.
  - **입장권을 쿼리로 보낸다.**
    - multipart 본문과 쿼리만 쓰는 요청은 CORS 사전 요청 없이 바로 간다.
    - 대신 쿼리가 접근 로그에 남으므로, 위의 로그 규칙으로 입장권 값을 `request_id`로 바꿔 적는다.
  - **이 경로만 본문 기한을 120초로 둔다.** 휴대폰의 느린 업로드를 고려했다. 입장권을 확인한 요청에만 적용한다.
  - **원래 파일명을 쓰지 않는다.** iPhone은 `image.jpg`·`IMG_1234.jpeg`처럼 겹치는 이름을 줘 충돌 처리가 필요하다. 내용 해시 이름은 같은 사진을 한 파일로 모으고, 다른 사진끼리 부딪히지 않는다.
  - **HEIC는 거절한다.** PC용 Anki가 HEIC를 보여 주지 못할 수 있다. iPhone에서 위젯으로 고른 사진은 이미 JPEG로 온다.
  - **위치정보(EXIF)는 처리하지 않는다.** 학습용으로 직접 찍은 사진을 올리는 일은 사실상 없다는 운영자 판단이다. 이미지 라이브러리도 추가하지 않는다.
  - **여러 장은 위젯이 한 장씩 차례로 올린다.**
    - 여러 파일을 한 요청으로 받는 서버 경로는 두지 않는다. 다건 원장과 부분 실패 처리가 서버에 필요하고,
      본문 한도(`maxRequestBodyBytes`)도 사진 몇 장이면 넘는다.
    - 장마다 새 입장권을 받는다. 서버 쓰기는 어차피 차례로 처리되므로 동시에 올려 얻는 것이 없다.
    - 한 장이 실패해도 나머지를 계속 올리고 그 줄에만 실패를 표시한다. 장수 상한은 두지 않는다(알림은 장마다 1건).
    - 결과는 모든 파일을 처리한 뒤 한 번만 알린다. ChatGPT는 답변을 생성하는 동안 온 자동 메시지를 보내지 않고,
      claude.ai는 컨텍스트를 모델에 넘기지 않으므로 실패도 메시지에 적는다.
  - **업로드한 이미지를 모델에게 보여 주는 기능은 두지 않는다.**
    - 평소에는 모델이 업로드 전에 이미지를 이미 안다(모델이 생성한 이미지, 채팅에 첨부해 합의한 이미지).
    - 채팅에 보여 주지 않고 올린 이미지는 모델이 볼 수 없으므로, 위젯의 안내 문구와 도구 결과 문장이 채팅 첨부를 안내한다.
      2026-09-25 lab에서 이런 사진의 내용으로 카드를 채워 달라고 하자, Claude는 2회 모두 볼 수 없다며 채팅 첨부를 요청했다.
      ChatGPT(Instant)는 2회 중 1회만 그렇게 했고, 1회는 사진과 무관한 카드 그림을 이미지 생성으로 만들었다(Anki에는 쓰지 않았다).
    - 위젯은 `ui/message`·`ui/update-model-context`에 이미지를 넣지 않는다. 두 클라이언트 모두 그 이미지를 모델에 넘기지 않았고,
      ChatGPT 모델은 받지 못한 이미지의 내용을 지어냈다.
    - 저장된 이미지를 도구 결과의 이미지로 돌려주는 방식은 Claude에서만 동작했다(원본 약 1.1 MB까지 통과, 2.4 MB는 실패).
      쓰려면 모델용 사본을 만들 이미지 라이브러리가 필요하다.
    - 채팅 없이 업로드부터 하는 일이 잦아지거나, 저장된 카드의 이미지를 모델이 다시 봐야 하거나, ChatGPT가 도구 결과의
      이미지를 모델에 넘기게 되면 다시 검토한다.
- **기준과 경위**
  - 현재 동작의 기준은 이 절과 테스트(`tests/anki-mcp/test_upload.py`, `test_server.py`, `test_metadata.py`)다.
  - 결정 경위와 실측 원자료는 GitHub 이슈 #1414(업로드 위젯), #1419(여러 장), #1420(채팅 첨부 흐름),
    #1421(모델이 이미지 보기, 구현하지 않음)에 있다. 이슈가 바뀌거나 사라져도 이 절만으로 동작과 이유를 알 수 있어야 한다.
  - 둘이 어긋나면 이 절과 테스트를 따른다.
...
`CS 재활 Basic`의 본문 크기는 모델 CSS(`managed-types/cs-rehab-basic/style.css`)의 `.card`가 정한다.
화면 폭 600px 이하는 16px, 그보다 넓으면 17px이다. 질문·답(1.1em), 맥락(0.8em), 출처(0.72em, 최소 12px)와
문장 안 `<code>`는 이 값에 비례한다. 코드 블록 크기(17px/15px)는 본문과 분리되어 있어 영향을 받지 않는다.

카드 ID 복사 버튼과 같은 줄, 버튼 왼쪽에 `가 − / 배율 / + / ↺` 조작부를 표시한다(`sync-addon/text-size-controls.html`).
규칙은 코드 블록 조작부와 같다. 80–140% 범위에서 10%씩 조절하고, `↺`는 현재 화면 폭의 기본값으로 되돌린다.
카드 ID가 같은 앞뒷면에서는 유지하고, 다음 카드 또는 답에서 질문으로 돌아갈 때 초기화한다.
유효한 CardID가 없는 미리보기는 같은 카드 ID 줄에서의 재실행만 유지한다.
브라우저 저장소·노트 필드·저장된 모델 정의에 배율을 기록하지 않는다. 본문과 코드 블록의 배율은 서로 독립이다.

배율은 `<html>`의 `--anki-text-scale` 변수로 전달하고, `.card`의 `calc(기본값 * var(--anki-text-scale, 1))`가 사용한다.
데스크톱 Anki는 `.card`를 `body`에 두고 카드마다 `body` class를 다시 쓰므로 `#qa`나 `body`에 변수를 두지 않는다.
카드 ID 스크립트가 렌더마다 자기 줄을 다시 채우므로, 이 조각은 앞면에서 카드 ID 조각 바로 뒤에
같은 카드 생성 조건 guard로 들어간다. 원장 검사는 이 순서나 guard가 어긋나면 실패한다. 뒷면은 FrontSide로 같은 조작부를 쓴다.
버튼에만 이벤트 전파를 차단하므로 복습 화면의 탭·키 동작은 그대로다.

앱 자체 기능은 이 조작부를 대신하지 않는다. 데스크톱 View 메뉴의 확대는 덱 목록까지 확대하고 재시작하면 초기화된다.
AnkiMobile의 Study Tools 글자 크기 버튼은 별도로 동작하며, 그 저장 범위는 확인하지 않았다.

DOM 테스트(`tests/anki-web/text-size-controls.test.mjs`)는 위치·범위·카드 전환·코드 배율과의 독립·이벤트 전파 차단을
검증하지만 실제 배치를 측정하지 않는다. Mac의 좁은/넓은 창과 iPhone의 세로/가로·야간 모드에서 글자 크기,
버튼 위치, 코드 조작부와의 공존, 복습 제스처를 따로 확인해야 한다. 배경과 결정은 #1426에 있다.
