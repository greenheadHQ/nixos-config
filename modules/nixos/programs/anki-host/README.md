...
2026-09-24 운영 배포 뒤, 이미 붙어 있는 태그를 다시 붙이는 요청(적용돼도 내용이 바뀌지 않는 쓰기)으로 관측했다.
6 Pro·5.5 Pro·5.6 Thinking 모두 첫 메시지에서는 미리보기에서 멈추고 사용자에게 확인을 물었다(응답 12–73초).
"이 요청 안에서 확인까지 하라"는 요청에도 6 Pro는 서버 안내를 들어 같은 턴의 확인을 보내지 않았다.
다음 메시지의 "적용해 줘"에서 다른 trace-id로 온 확인이 적용됐고, 미리보기부터 적용까지 54초였다.
모델이 같은 턴의 확인을 보내지 않아 서버의 거절 경로 자체는 실측하지 못했다.
