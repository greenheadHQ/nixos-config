...
MCP 변경 알림은 한국어 작업명과 처리 결과, AnkiWeb 동기화 상태를 먼저 보여 준다. 숫자는 처리 대상이며
변경된 개수를 뜻하지 않는다. 노트 추가에서 확인된 추가 수는 `result.added`로 따로 표시한다.
알림 마지막의 **문제 문의용 작업 번호**는 전체 `operation_id`이며, 비슷한 작업이 여러 건이거나 시간이
지난 뒤에도 `anki_operation_status`로 정확한 결과를 찾는 데 사용한다. 카드 ID와 다르다.
본문·덱명·태그명·파일명·오류 상세는 알림에 넣지 않는다. AnkiWeb 동기화 완료는 다른 기기의 수신 완료를
