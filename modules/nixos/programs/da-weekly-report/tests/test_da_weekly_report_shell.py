...
def _reset_publish_records(fixture: dict, *, github: str, pushover: str) -> None:
...
def _existing_final_fixture(tmp_path: Path, weekly_report_module) -> dict:
    # Reuse the unit-test fixture so the shell path exercises all six metrics,
    # 10,000 raw path warnings, and multibyte commentary.
    from test_weekly_report import projection_stress_report
...
def _run_report(fixture: dict) -> subprocess.CompletedProcess[str]:
...
def test_initial_run_passes_exact_bounded_projection_to_llm(
    ...
    from test_weekly_report import sample_sidecar
