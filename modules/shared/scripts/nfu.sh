...
echo ""
log_info "═══ 업데이트 완료 ═══"
log_warn "⚠️  FOD hash는 현재 플랫폼($(uname -s))의 config만 검증됐습니다."
log_warn "    다른 플랫폼 전용 FOD(예: NixOS의 caddy.withPlugins)는 미검증 상태입니다."
echo "💡 변경사항을 커밋·푸시하세요 (2번째 호스트가 pull로 받습니다):"
echo "   git add -u && git commit -m 'chore: update flake inputs' && git push"
echo ""
echo "💡 2번째 호스트에서는 nfu가 아니라 아래를 실행하세요:"
echo "   git pull && ./scripts/fix-fod-hashes.sh && nrs"
echo "   # hash가 수정됐다면 반드시 저장소로 되돌리세요 (안 하면 다음 nfu가 dirty tree로 중단):"
echo "   git add -u && git commit -m 'fix: <platform> FOD hash' && git push"
echo "   (nfu는 nix flake update를 선행하므로, 2번째 호스트에서 실행하면"
echo "    방금 검증·커밋한 lock이 다른 rev로 재갱신됩니다)"
