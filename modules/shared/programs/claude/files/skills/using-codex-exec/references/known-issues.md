...
| 자식 sandbox denial (`-s read-only`에서 쓰기 시도) | rc 0으로 끝나고 결과 파일도 non-empty이므로 rc·파일 검사로는 못 잡는다 — 완료 표식 grep과 결과 본문의 자기신고 문구로 판정. stderr 분류 시에는 `ERROR <module>: error=patch rejected: writing is blocked by read-only sandbox` 형태를 찾는다 (리터럴 `ERROR:` 0건) | 2026-08-15, 0.147.0 실측 |
