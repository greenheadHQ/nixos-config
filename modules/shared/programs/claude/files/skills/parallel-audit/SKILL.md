...
- 일회성 강제: "`SAFE`가 나올 때까지" 같은 지시에도 같은 changeset에 자동으로 반복 재발사하지 않는다. 1회 전수조사 후 결과를 보고하고, fix 후 재검증이 필요하면 사용자 확인을 거쳐 새 단발 조사로 실행한다. 매 재조사가 새 리뷰 표면을 만드는 비수렴이 의심되면(전수조사는 일회성이라 자동 라운드 카운팅이 없다) 자동 재발사 대신 사용자에게 보고하고 판단을 구한다.
