...
1. 제목·본문·라벨을 검토한다. 기존 요청으로 게시가 승인되었으면 진행하고, 게시 대상이나 작업 내용이 불명확할 때만 필요한 판단을 묻는다.
...
이행 가이드가 이미 요청된 경우 `/write-handoff <생성된 ISSUE_URL>`로 이어간다. 요청되지 않았다면 이슈 URL을 반환하고 종료한다. 이후 사용자가 요청하면 같은 URL을 전달한다. bare 번호 대신 확인된 URL을 사용한다.
