...
- 예외: PR 생성 전 초안 시점의 현재 작업 중인 버전은 PR 번호가 아직 없으므로 `(이번 변경)`으로 표기한다 (PR 본문에 그대로 게시 — 머지 후에는 자기 PR로 자명).
- 검토 라운드 번호, finding ID(`Correctness-1`, `CORR-2` 등)는 CIR에 포함하지 않는다 (휘발성 보고용).
- 최종 결정의 trade-off를 솔직하게 명시한다.
- 제거·되돌림이면 무엇을 왜 제거/되돌리는지와, 그 대상이 과거에 의도적으로 도입된 것이면 도입 근거·출처(PR#/commit/issue#)를 명시한다 (미래 세션의 회귀 재도입 방지 — 회귀 조사가 읽을 신호).
...
- 데모·터미널 증빙 이미지가 있으면 [시각 증빙 첨부 절차](../../attaching-github-media/SKILL.md)에 따라 첨부한다.
...
- 시각·UI 변경에서 명시적으로 제공된 before/after 이미지 후보가 있으면 [시각 증빙 첨부 절차](../../attaching-github-media/SKILL.md)에 따라 첨부한다. 후보가 없으면 첨부 절차를 실행하지 않는다 (임의 이미지 생성·업로드 금지).
