...
포맷·크기 지원 여부는 스킬이 선제 판정하지 않고 GitHub 서버에 위임한다 — 서버가 거부하면 `FAILED(PREUPLOAD)`로 기록하고 본 작업은 fail-open으로 계속한다. 다음처럼 이 스킬의 surface·역할 밖인 요청은 업로드 없이 미지원 사유만 안내한다.
...
호출 스킬이 후보 목록, 본문 사본, 삽입 슬롯, 게시 게이트를 제공한다. 후보의 식별 기준은 호출 스킬 문서가 정의한다 — 현행 create-issue/create-pr는 시각 증빙 중심이며, 소비자 경로의 비이미지 확장은 소비자 스킬의 후속 결정 사항이다.
