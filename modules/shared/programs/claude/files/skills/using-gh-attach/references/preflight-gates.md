...
직접 검사의 공통 원칙은 하나다 — "이 파일의 전체 내용을 직접 열람해 확인했는가". 파일 경로·메타데이터·바이트 존재 검사만으로는 이 원칙을 충족하지 않으며, 파일 종류별 구체 확인 형태는 아래 표를 따른다. 분류 입력은 magic MIME, (PNG의 경우) 사전 게이트 6단계 파서 판정, (`text/*`의 경우) 아래 렌더링 마크업 마커 판별이다.

| 분류 | 검사 수단 | 확인 형태 |
|------|-----------|----------|
| 정적 이미지 (JPEG, 6단계 파서로 `acTL` 부재가 확인된 정적 PNG) | 이미지 렌더링 열람 | 픽셀 시각 확인 |
| 플레인 텍스트 (`text/*` 중 렌더링 마크업 마커가 없는 코드·로그·CSV·JSON 등) | 내용 읽기 | 전체 내용 확인 |
| PDF | 페이지 읽기 (수단이 있는 런타임) | 전체 페이지 확인 |
| 애니메이션 가능 이미지 (GIF·WebP)·렌더링 마크업 (`image/svg+xml`·`text/html`·마커 판별에 걸린 `text/*`)·동영상·오디오·압축·기타 바이너리·APNG | 없음 — 직접 검사 불가 | 아래 사용자 확인 게이트 |
