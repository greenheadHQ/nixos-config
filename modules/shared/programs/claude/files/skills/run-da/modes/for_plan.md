...
- codex exec 경로: 선택된 review unit(FULL 기본 4개, LITE는 선택한 수, explicit exhaustive는 6개) 전부 실행(Claude Code는 병렬, headless는 serial) 완료 후, 각 `$DA_DIR/$UNIT-result.md` 패턴의 결과 파일을 파일 읽기 도구로 명시적으로 읽어 수집한다. 실패 판정 순서는 rc(`$DA_DIR/$UNIT.rc` — 부재는 실패) → 결과 파일 존재·비어 있지 않음 → stderr는 원인 분류 전용이다 ([`../references/arbiter-scaling.md`](../references/arbiter-scaling.md) rc 계약). 예외: `service_tier`가 주입된 호출은 성공(rc 0) unit에 대해서도 이 수집 시점에 `$DA_DIR/$UNIT-stderr.log`의 tier 경고를 확인한다 — tier는 잘못된 값이 rc로 드러나지 않는 축이라 성공 경로 검사가 없으면 조용히 무시된다 (계약 정본: arbiter-scaling "사용자 지정 실행 파라미터"의 tier 무시 감지).
