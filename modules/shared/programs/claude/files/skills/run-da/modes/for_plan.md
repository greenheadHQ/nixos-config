...
- 5b. Selective consistency trigger 검사: first-pass 결과의 VERDICT_JSON 블록을 읽어 [`../references/stability-measurement.md`](../references/stability-measurement.md)의 trigger 조건에 매치되는 finding을 식별한다 (조건 정의는 해당 문서가 SSOT). VERDICT_JSON을 읽는 이 지점(과 5d의 N=3 수집)에서 schema 1.1 caller 검증(`axes.plausibility` 정합 행렬 + `accepted_severity`/`reviewer_severity`/`rejection_basis` 정합 + 실시간 경로 schema ≥1.1)을 함께 수행한다 — 검증 규칙과 fail-closed 전이는 [`../references/protocol.md`](../references/protocol.md) "수렴 판정"이 SSOT.
