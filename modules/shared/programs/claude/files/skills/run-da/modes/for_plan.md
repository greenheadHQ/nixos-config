...
- 수집한 각 finding의 ID가 canonical 형식(`{BUNDLE}-{순번}` — 정규식 `[A-Za-z_]+-[0-9]+`)과 일치하고 결과 내에서 유일한지 검사한다. 위반 finding이 있으면 해당 unit을 recoverable violation으로 폐기하고 fresh 실행 단위로 1회 재실행한다 — reviewer 산출 ID는 비신뢰 입력이며 이후 `--expect-findings` 셸 인자로 전달되므로, 검증 통과 ID만 manifest에 조립한다 (shell-safe 문자 집합 보장).
