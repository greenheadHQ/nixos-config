...
- 리뷰 대상 무결성 manifest (#1259): diff를 파일로 저장하는 실행 형태(reviewer가 diff 파일을 직접 읽는 codex exec 경로)에서는 저장 직후 manifest를 계산해 reviewer 프롬프트에 명시한다 — ①`git merge-base main HEAD`와 `git rev-parse HEAD`의 SHA, ②`shasum -a 256 < <diff파일>`, ③`wc -l < <diff파일>`. reviewer에게 ②③을 같은 명령으로 재계산해 대조하고, 같은 checkout에서 `git diff main...HEAD`를 재생성한 해시·줄수도 전달 파일과 비교하도록 지시한다 — 어느 대조든 불일치면 리뷰를 진행하지 않고 `VIOLATION (manifest mismatch)`으로 보고한다. 전달 파일 해시만 대조하면 stale하게 생성된 diff를 그대로 해시해 전달하는 사고(도입 근거 실측의 stale 커밋 리뷰)를 못 잡는다 — ref SHA와 재생성 대조가 그 공백을 닫는다. 해시 입력은 diff 파일 바이트 그대로다(개행 변환·경로 재작성 없음 — 정규화 규칙은 "가공 금지" 하나다). 프롬프트에 diff를 인라인 포함하는 형태는 전달과 대상이 동일 채널이라 manifest를 생략한다.
