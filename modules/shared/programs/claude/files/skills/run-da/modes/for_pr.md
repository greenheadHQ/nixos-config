...
- 경로 상태 정의: write phase는 세 경로 집합을 구분해서 다룬다. 하나로 합치면 "건드려도 되는 곳"과 "실제로 바뀐 곳"이 섞여, 계획했다가 결국 수정하지 않은 경로가 finalize 대조를 실패시키거나 승인 경로만 예외 처리할 수 없게 된다.
  ...
  - `write_phase_delta`: write phase가 workspace에 만든 모든 변경 경로. `authorized_paths` 필터를 거치지 않고 먼저 산출한다 — 필터를 먼저 걸면 승인 밖 경로가 부분집합 검사에서 보이지 않아 commit 이후에야 드러난다. 두 소스를 합친다: tracked 변경은 `git diff --name-only --no-renames -z <pre_write_sha> --`(범위 표기 `..` 없이 — `<sha>..`는 `<sha>..HEAD`로 해석되어 작업트리를 보지 않으므로 write phase의 미커밋 변경이 전부 빠진다), untracked 신규는 `git status --porcelain=v1 -z --untracked-files=all`의 `??` 항목(`git diff`는 untracked를 보지 않는다). 여기서 baseline의 기존 dirty/untracked 경로는 제외하되 `approved_mixed_paths`는 남긴다.
  - `actual_commit_paths`: 승인 범위 검사를 통과한 `write_phase_delta`. 검사 자체가 `write_phase_delta ⊆ authorized_paths`이므로 두 집합은 그때 같아진다 — stage·commit·사후 대조의 기준은 계획이 아니라 이 집합이다. walkthrough가 앞선 수정을 되돌려 최종 delta가 0인 경로는 애초에 `write_phase_delta`에 없다. rename은 `--no-renames`로 source와 destination 두 경로로 펼쳐지므로 사후 대조와 표현이 일치한다.
...
- finalize (walkthrough CLEAN 후) — 아래 체크를 순서대로 수행한다:
  1. 최종 diff 확인.
  2. 승인 범위 검사: 위 정의대로 `write_phase_delta`를 산출하고(승인 필터 없이 workspace 실제 변경 전체), 이것이 `authorized_paths`의 부분집합인지 확인한다. 초과 경로가 있으면 승인 밖 변경이므로 — formatter/generator가 예상 밖 파일을 만든 경우가 대표적이다 — commit하지 않고 중단·보고한다. 통과하면 그 집합이 `actual_commit_paths`가 된다. 이 검사만이 commit 생성 전에 동작하는 경계이므로, 검사 대상을 승인 목록으로 먼저 좁히지 않는 것이 요점이다.
