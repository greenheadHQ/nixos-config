...
- 경로 상태 정의: write phase는 네 경로 집합을 구분해서 다룬다. 하나로 합치면 "건드려도 되는 곳"과 "실제로 바뀐 곳"이 섞여, 계획했다가 결국 수정하지 않은 경로가 finalize 대조를 실패시키거나 승인 경로만 예외 처리할 수 없게 된다. 앞의 둘(`authorized_paths`·`approved_mixed_paths`)은 write phase 진행 중 갱신되는 승인 상태이고, 뒤의 둘(`write_phase_delta`·`expected_commit_paths`)은 finalize 시점에 관측·파생되는 값이다.
  ...
  - `write_phase_delta`: write phase가 workspace에 만든 모든 변경 경로. `authorized_paths` 필터를 거치지 않고 먼저 산출한다 — 필터를 먼저 걸면 승인 밖 경로가 부분집합 검사에서 보이지 않아 commit 이후에야 드러난다. 두 소스를 합친다: tracked 변경은 `git diff --name-only --no-renames -z <pre_write_sha> --`(범위 표기 `..` 없이 — `<sha>..`는 `<sha>..HEAD`로 해석되어 작업트리를 보지 않으므로 write phase의 미커밋 변경이 전부 빠진다), untracked 신규는 `git status --porcelain=v1 -z --untracked-files=all`의 `??` 항목(`git diff`는 untracked를 보지 않는다). baseline의 기존 dirty/untracked 경로는 경로명만으로 빼지 않고 저장해둔 baseline diff·hash와 현재 내용을 대조한다 — 내용·존재·mode가 그대로면 write phase가 건드리지 않은 것이므로 제외하고, 달라졌으면 `approved_mixed_paths`가 아닌 한 write phase가 승인 없이 사용자 파일을 변경한 것이므로 이 집합에 포함시킨다 (경로명으로 제외하면 formatter/generator의 사용자 파일 훼손이 commit 전 검사에 보이지 않고, untracked는 hash만 저장하므로 덮어쓴 원본을 복구할 수도 없다).
  - `expected_commit_paths`: 승인 범위 검사를 통과한 `write_phase_delta` (commit 전에 확정되는 값이라 "실제 commit된 경로"가 아니다 — 그것은 finalize 마지막에 `git diff-tree`로 관측하는 `committed_paths`이고, 둘의 일치를 확인하는 것이 사후 대조다). 검사 자체가 `write_phase_delta ⊆ authorized_paths`이므로 통과 시 `expected_commit_paths`는 `write_phase_delta`와 같다. stage·commit의 기준은 계획이 아니라 이 집합이며, walkthrough가 앞선 수정을 되돌려 최종 delta가 0인 경로는 애초에 `write_phase_delta`에 없다. rename은 `--no-renames`로 source와 destination 두 경로로 펼쳐지므로 `committed_paths`와 표현이 일치한다.
  - 경로 정규화 (네 집합 공통): 모든 원소는 repo-relative canonical leaf 파일 경로여야 한다. `./foo`와 `foo` 같은 별칭, 절대경로, `..`가 든 경로는 정규화하고, 디렉터리 경로는 받지 않는다 (leaf 파일로 펼친다). 정규화 없이 문자열 집합만 비교하면 별칭이 겹침 검사를 통과한 뒤 Git pathspec으로는 같은 파일을 선택하고, 디렉터리 pathspec은 그 아래 사용자 파일까지 stage·commit한다.
...
- finalize (walkthrough CLEAN 후) — 아래 체크를 순서대로 수행한다:
  1. 최종 diff 확인.
  2. 승인 범위 검사: 위 정의대로 `write_phase_delta`를 산출하고(승인 필터 없이 workspace 실제 변경 전체), 이것이 `authorized_paths`의 부분집합인지 확인한다. 초과 경로가 있으면 승인 밖 변경이므로 — formatter/generator가 예상 밖 파일을 만든 경우가 대표적이다 — commit하지 않고 중단·보고한다. 통과하면 그 집합이 `expected_commit_paths`가 된다. 이 검사만이 commit 생성 전에 동작하는 경계이므로, 검사 대상을 승인 목록으로 먼저 좁히지 않는 것이 요점이다.
  - 빈 집합 분기: `write_phase_delta`가 비면 아래 stage·commit·commit 경로 대조를 생략한다 (`git commit --only`는 경로 없이 호출할 수 없다 — 실측 `fatal: No paths with --include/--only does not make sense.`). 이 상태는 walkthrough가 batch 수정을 전부 되돌렸다는 뜻이므로 round_write_set이 해결됐다고 보지 않는다: 되돌린 이유를 walkthrough 후속 발견으로 기록하고, 해당 finding 수를 [`../references/protocol.md`](../references/protocol.md)의 `write_reverted_count`로 확정한다 (그 값이 0이 아니면 수렴 predicate가 막힌다). 새 changeset 선언도 하지 않는다.
  3. 신규 경로 stage: `expected_commit_paths` 중 Git이 아직 추적하지 않는 신규 파일·rename destination만 `git --literal-pathspecs add -- <해당 경로>`로 제한적으로 stage한다. path-limited commit은 Git이 이미 아는 경로만 커밋할 수 있으므로, 이 선행 없이는 신규 파일을 만드는 batch(테스트·fixture·generated output)의 commit이 pathspec 오류로 실패한다. 무관한 기존 staged 항목은 건드리지 않는다.
  4. commit: 메인 에이전트가 single-writer로 `git --literal-pathspecs commit --only -- <expected_commit_paths>` ([`../references/hardening-contract.md`](../references/hardening-contract.md)의 single-writer 정의. 경로 한정 커밋은 기존 index의 무관한 staged 사용자 항목을 포함하지도, 건드리지도 않는다 — 전역 index equality를 요구하면 무관한 staged 변경이 finalize를 영구 차단한다).
  5. commit 경로 대조: 생성된 commit에서 관측한 `committed_paths`가 `expected_commit_paths`와 일치하는지 확인한다. `committed_paths`는 `git diff-tree --no-commit-id --name-only --no-renames -r -z HEAD`로 얻어 NUL 단위로 비교한다 — rename detection이 켜진 출력(`git show --name-only`)은 rename을 destination 하나로 접어 source 경로를 숨기므로, batch가 rename을 포함하면 정상 commit도 집합 불일치로 오판된다. `--no-renames`는 rename을 delete+add 두 경로로 펼쳐 `expected_commit_paths`와 같은 표현을 만들고, `-z`는 newline이 든 경로가 두 원소로 쪼개지는 것을 막는다.
