...
| Step 6 write phase | 통합 반영 루프(통합 설계→batch 반영→walkthrough→후속 수정 처리→finalize) 후 계획 확정·새 changeset 선언 | 동일 루프를 코드에 적용하되, commit은 finalize 단계에서 walkthrough CLEAN 후에 수행한다: walkthrough CLEAN → 최종 diff 확인 → 메인 에이전트가 single-writer로 commit ([`../references/hardening-contract.md`](../references/hardening-contract.md)의 single-writer 정의) → `git status --porcelain` clean 확인 → 새 changeset(diff/commit range) 선언 + 변경 범위를 round summary에 기록. walkthrough 후속 수정이 uncommitted로 남아 push에서 누락되거나 다음 라운드 clean preflight를 깨는 것을 구조적으로 방지한다 |
