...
### 검증기 공통 계약 (산출 주체 중립 — reviewer·Arbiter 공통)
...
검증 대상 원본 고정 (산출 주체 중립 — reviewer·Arbiter 공통): 검증기 입력은 실행 단위가 산출한 결과 파일의 원본 경로다. 결과 내용을 손으로 옮겨 적은 전사본이나 heredoc으로 재구성한 파일을 검증 대상으로 삼는 것은 검증 우회이며 금지다 — 실측에서 첫 라운드만 원본을 검증하고 이후 전 라운드가 손 전사본을 검증해 전사 오류가 반복 유입됐다(#1259). 원본 파일이 유실됐으면 검증 실패로 처리하고 해당 실행 단위를 재실행한다 (전사로 복구하지 않는다).
