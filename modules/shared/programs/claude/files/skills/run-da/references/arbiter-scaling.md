...
- 5회 라운드 초과 또는 추세 기반 조기 중단([`protocol.md`](protocol.md)의 "최대 라운드 수" 정의) 도달 시 질문 도구 불가 → 미해결 CONFIRMED finding이 남아 있으면 3회 반복 규칙의 자동 수용과 동일하게 먼저 자동 수정한 뒤 DA 루프를 종료한다. 종료 시 미해결 CONFIRMED가 없으면 CLEAR로 간주하고, 남아 있으면(추세 비수렴 등) 비수렴 상태로 보고 후 종료한다.
