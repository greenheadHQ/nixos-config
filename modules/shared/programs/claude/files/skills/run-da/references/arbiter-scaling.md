...
- 추세 기반 조기 중단([`protocol.md`](protocol.md)의 "최대 라운드 수" 정의) 도달 시 질문 도구 불가 → 비수렴(unconverged)으로 종료한다. 추세 비수렴은 매 라운드 수정이 새 표면을 만드는 상태이므로 추가 자동 수정을 시도하지 않고([`protocol.md`](protocol.md)의 changeset 동결/범위 축소 권고를 따른다), 현재 미해결 상태를 비수렴으로 보고한 뒤 종료한다 — CLEAR로 간주하지 않는다.
