...
- Arbiter는 단일 exec (병렬 fan-out 없음): reviewer만 병렬 실행한다 (런타임별 병렬 실행 매커니즘은 위 표 참조). 발사 방식은 세션 유형별로 다르다 — headless는 serial foreground, Claude Code 세션은 하네스 foreground 상한 때문에 `run_in_background: true`로 발사 후 완료 알림으로 수집한다 (정본: [`arbiter-scaling.md`](arbiter-scaling.md) codex exec 경로 실행 계약).
