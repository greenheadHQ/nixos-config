...
- fork 상속 금지 (DA 실행 단위 공통): full-history fork는 부모의 대화 이력과 model/effort를 상속하므로 DA reviewer/Arbiter spawn에 사용하지 않는다 — spawn 격리 계약(빈 컨텍스트 시작)과 Arbiter 강도 하한([`arbiter-scaling.md`](arbiter-scaling.md) 정본)을 동시에 깨뜨린다. 실측에서 native 경로의 Arbiter가 fork 상속으로 reviewer보다 낮은 강도로 도는 역전이 다수 관측됐고(#1258), 부모의 이전 라운드 reviewer 원문·판정·수정 diff를 통째로 상속한 사례가 관측의 절반이었다(#1259). DA 실행 단위는 어떤 경로에서든 빈 컨텍스트 시작이 spawn 격리 계약이다 — Codex native의 reviewer·Arbiter `spawn_agent` 호출은 `fork_turns: "none"` 명시가 필수다 (생략·`"all"`은 부모 이력을 상속하므로 기본값에 맡기면 침묵 위반이 된다) (`fresh` modifier와 별개 축 — modifier는 이전 라운드 내용을 프롬프트에 넣지 않는 선택 동작이고, spawn 격리는 실행 단위가 부모 대화를 상속하지 않는 필수 규칙이다) — Codex native는 fresh spawn + effort 명시 설정만, Claude 경로는 컨텍스트를 상속하는 fork형 subagent를 쓰지 않는다. 하네스가 spawn 설정을 노출하지 않아 실행 전 확인이 불가능한 경로에서는 산출물 신호를 보조 감지로 사용하되, 신호는 "이번 입력(프롬프트·diff·참조 가능한 파일·git 이력)에 존재하지 않는 이전 라운드 산출물의 원문 인용 — 메인이 부여한 이전 라운드 suffix ID의 인용 포함"으로 한정한다. 새 결과의 `ID` 라벨 값은 비교 대상이 아니다 — reviewer ID는 매 결과 1부터 재시작하므로 기본형 ID의 재등장은 정상이고, 후속 라운드 reviewer가 누적 diff와 git 이력에서 읽은 수정 내용의 일반 언급도 상속 증거가 아니다. 한정된 신호가 감지되면 해당 unit은 spawn 격리 위반으로 폐기한다 — 같은 불투명 경로로 재실행해도 상속 조건은 달라지지 않으므로 반복하지 않고, 격리를 명시할 수 있는 경로(codex exec 등)로 전환을 사용자에게 고지·확인받거나 전환 불가면 해당 unit을 BLOCKED로 남긴다. 비교 기준은 메인이 보존한 이전 라운드 산출물(원문·부여한 suffix ID)이다.
