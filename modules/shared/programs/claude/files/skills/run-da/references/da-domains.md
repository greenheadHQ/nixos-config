...
- 허용 문자: prefix는 영문자와 `_`, 순번은 숫자, 선택적 suffix는 `-r{숫자}` (정규식 `[A-Za-z_]+-[0-9]+(-r[0-9]+)?`). 이 제약의 목적은 namespace 검증이 아니라 shell-safe 보장이다 — reviewer가 만든 ID는 비신뢰 입력인데 이후 `--expect-findings` 셸 인자로 전달된다.
