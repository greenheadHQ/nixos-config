...
- Evidence, not ritual. Validation 항목은 "절차를 돌렸다"는 표시가 아니라, 변경이 실제로 의도대로 동작한다는 증거다.
...
- 대상: browser-capable skill(예: 외부 Playwright CLI)로 수행하는 exploratory 또는 scripted 검증.
...
2. 그 risk가 실제로 발생하는 표면을 식별한다 (DB schema, HTTP handler, React DOM, observability surface...).
