...
사용자에게 질문 도구로 판단을 요청할 때 (3회 반복 규칙, 라운드 한계효용 저하, 5회 라운드 초과, fresh 모드 반복 감지, remediation_scope UNCLEAR 판단 등 모든 경우), 사용자가 딴짓을 하다가 돌아온 상황을 가정하고 다음을 모두 포함한다:
...
> - 배출(DEFERRED): 별도 이슈로 이관하고 현재 루프에서 제외"
