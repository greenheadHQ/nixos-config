...
- Round outcome 스냅샷 기록과 accepted severity 집계: write phase 진입 직전 round outcome 스냅샷을 고정하고, VERDICT_JSON 수집 시 schema 1.1 caller 검증(`axes.plausibility` 정합 행렬 + `accepted_severity`/`reviewer_severity`/`rejection_basis` 정합 + 실시간 경로 schema ≥1.1)을 수행하며, accepted severity의 집계(최댓값 계산)만 담당한다 — 값 산출은 Arbiter 소관이다. 규칙 정본은 [`protocol.md`](protocol.md)의 "수렴 판정".
