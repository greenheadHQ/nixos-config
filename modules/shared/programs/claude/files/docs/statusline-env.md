...
실패). 그 결과 rate_limits 의 progressive disclosure 가 가장 좁은 단계로 고정된다.
...
| raw COLS | EFF_COLS | rate_limits detail |
|----------|----------|-------------------|
| 50 | 50 | detail=2 |
| 80 | 40 | detail=2 |
| 100 | 60 | detail=3 |
| 128 | 88 | detail=4 |
| 140 (default) | 100 | detail=4 |
| 200 | 160 | detail=4 |
...
| split pane (~80 cols, 비-SSH) | 80 (또는 unset → default 140 — wrap 위험 감수) |
| 좁은 ssh 세션 (~80 cols 이상) | unset → default 140 (SSH 분기에서 rate_limits gauge 가 vertical bracket `▏…▕` 으로 3-cell 압축되어 한 줄 wrap 위험 낮음). 실제 폭이 80 cols 미만이면 명시 override 권장. |
