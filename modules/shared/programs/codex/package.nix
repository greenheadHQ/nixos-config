...
# 신뢰 없이 최신 codex를 받기 위한 self-maintained overlay다. codex-rs는 정적 단일 바이너리
...
stdenvNoCC.mkDerivation {
  ...
  installPhase = ''
    ...
    # 현행 codex 릴리스 tarball은 단일 바이너리(codex-<triple>; flat 또는 triple-named subdir)만
    # 담는다. 추출 정규파일이 정확히 1개임을 단언해, upstream이 다파일 레이아웃(예: 바이너리+README)으로
    # 바꾸면 silent misinstall 대신 loud fail 하도록 한다(그때 maintainer가 선택 로직을 갱신).
    codex_bin="$(find unpacked -type f)"
    if [ "$(printf '%s\n' "$codex_bin" | grep -c .)" -ne 1 ]; then
      echo "codex: 추출 정규파일이 1개가 아님 — 릴리스 레이아웃 변경. package.nix install 로직 갱신 필요:" >&2
      printf '%s\n' "$codex_bin" >&2
      exit 1
    fi
    install -Dm755 "$codex_bin" "$out/bin/codex"
    runHook postInstall
  '';
