...
| `scenario-G-removed-template-leaf-residue.toml` | template에서 삭제된 앱 소유 leaf(2026-09-05 model pin 제거)가 기존 배포본에 남는지 | `model` / `model_reasoning_effort` 잔재 보존 + 템플릿 재선언 시 실패 + template-owned leaf(`approval_policy`, `features.goals`) 강제. 회수 목록(`retired-config-keys.txt`)에 등록된 퇴역 키의 `--unset` 동작은 `tests/suites/codex-config.sh`가 소유 |
