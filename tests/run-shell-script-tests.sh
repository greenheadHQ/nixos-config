...
# 수동 실행(`bash tests/run-shell-script-tests.sh`)과 required CI의 통합 runner가 동일한 hermetic
# runtime(flake-pinned `.#prePushRuntime` profile + GNU coreutils/findutils, #1009)을 쓰도록 강제한다.
set -euo pipefail
