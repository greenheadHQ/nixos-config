...
#   analyzing-da-sessions-tests · flake-check · statusline-bats) + eval-tests + 어느 훅에도 미연결된 tests/test-*.sh 단위
...
# 7) flake-check — 전 시스템 flake 평가 게이트(repo 전역). eval-tests의 선택적 평가가 강제하지
...
# 8) statusline-bats — statusline Bats 테스트. nixpkgs#bats를 nix shell로 제공하고 TERM을 주입한다.
...
# 9) precommit-staged-snapshot — 어느 훅에도 연결되지 않은 수동 전용 드라이버를 통합에 포함한다.
