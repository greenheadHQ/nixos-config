...
대부분의 시간이 네트워크 I/O에 소비됩니다 (CPU 사용률이 6% 정도로 매우 낮음).
...
`/etc/nixos/configuration.nix`는 flake 시스템에서 사용되지 않는 레거시 파일입니다. 이 파일을 수정해도 빌드에 영향이 없고, 전통적 빌드 명령은 NIX_PATH 오류를 발생시킵니다.
