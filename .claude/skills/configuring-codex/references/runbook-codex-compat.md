...
- 실측 codex 런타임과 `codex-pin.json`의 버전 일치 확인: `jq -r .version modules/shared/programs/codex/codex-pin.json && codex --version` (버전 숫자는 `SKILL.md` "작성 기준" 스탬프가 SoT — 여기 박지 않아 bump 때 stale해지지 않게 한다).
