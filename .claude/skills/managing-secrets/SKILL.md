...
| MiniPC toss CLI opnix materialize (#1044 전까지 `homeserver.toss.enable = false`로 비활성 — 모듈만 준비됨) | 1Password | Automation | `토스증권 Open API` → `/run/opnix/<user>/toss-client-id`, `/run/opnix/<user>/toss-client-secret` |
...
| `opnix-service-account-token.age` | agenix → 1Password SA | Automation (SA 접근 vault) | `/run/agenix/opnix-service-account-token` (`root:onepassword-secrets`, 0640, MiniPC) | opnix tokenFile → 부팅 oneshot이 `op://Automation/github-pat/token`을 `/run/opnix/<user>/github-pat`으로 materialize. 토스 credential(`/run/opnix/<user>/toss-client-{id,secret}`)은 #1044 전까지 `homeserver.toss.enable = false`라 materialize되지 않음 | minipcHostOnly (host key 복호화) |
...
| SA token (minipc) | 1Password | Automation (read-only) | 1Password Service Account; token은 `opnix-service-account-token.age`로 암호화되어 `/run/agenix/opnix-service-account-token` 배포 | MiniPC opnix가 github-pat materialize 시 사용. 토스 credential materialize는 #1044 전까지 `homeserver.toss.enable = false`로 비활성. blast radius=Automation read-only(github-pat + 토스증권 OpenAPI credential — SA가 읽을 수 있는 범위는 게이트와 무관). 90일 cadence rotation (만료 record†). Mac SA와 별개 발급 격리 SA | — |
