...
> 참고: `activateSettings -u`는 `마우스` > `자연스러운 스크롤` 옵션을 활성화시키는 부작용이 있어, 직후에 `defaults write`로 재설정합니다.
...
서드파티 tap의 formula는 전체 경로로 지정해야 자동 설치됩니다:
...
| VSCode  | `pkgs.vscode` (+ `nix-vscode-extensions` overlay) | 코드 에디터 |
...
| Shottr 스크린샷 폴더 (`~/FolderActions/upload-immich`) | Immich 자동 업로드 + Pushover 알림 (personal 전용) |
...
- 격리 시 Pushover 알림이 절대경로와 함께 best-effort로 발송된다
  (`pushover-folder-actions` credential이 활성이면 발송, 부재/네트워크 실패는
  log_warn만 남기고 silent skip).
- 격리 디렉토리는 watch root 밖이므로 launchd `WatchPaths`를 추가로 트리거하지 않는다.
- `wait_file_stable`이 timeout으로 false를 반환한 파일은 같은 run에서는 deferred
  처리되고 격리되지 않는다. 단 launchd `WatchPaths`는 watch dir 변경 시에만
...
2. 원인을 파악한 뒤 (ffmpeg 로그는 `~/Library/Logs/folder-actions/*.error.log` 참조),
   파일명 prefix(`<timestamp>_<pid>_<rand>_`)를 제거하고 watch dir로 다시 옮기면 재처리.
...
Shared HM 모듈로 관리하는 CLI 도구:

| 패키지 | 모듈 경로 | 비고 |
|--------|-----------|------|
| `broot` | `modules/shared/programs/broot/` | Darwin + NixOS 공통 |
