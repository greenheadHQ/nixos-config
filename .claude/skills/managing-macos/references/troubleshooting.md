...
`darwin-rebuild switch`는 시스템 수준 설정을 변경하기 때문에 root 권한이 필요합니다.
...
bash에서 `((expression))`의 exit code는 표현식의 평가 결과에 따라 결정됩니다:
...
`((var++))`는 후위 증가로, 증가 전 값(0)을 반환합니다. `set -e` 환경에서 exit code 1은 스크립트를 즉시 종료시킵니다.
