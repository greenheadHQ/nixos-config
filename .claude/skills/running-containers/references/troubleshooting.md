...
Scriptable 스크립트 설정에서 Share Sheet Inputs의 Images만 체크한 경우 발생할 수 있음.
...
Redis의 `stop-writes-on-bgsave-error=yes` 옵션으로 인해, RDB 저장 실패 시 데이터 일관성 보호를 위해 모든 쓰기 작업이 차단됨. 이로 인해 Immich의 Job Queue 추가가 실패하여 웹 UI의 모든 Job 관련 기능이 작동하지 않음.
