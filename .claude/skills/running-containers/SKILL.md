...
description: |
  Manage Podman containers and shared NixOS homeserver infrastructure: Caddy, networking, updates, backups, and resource limits.
  Service-specific application workflows use hosting-copyparty or hosting-karakeep.
---
...
| Anki (headless) | `anki-host-backup` | 04:15 (`homeserver.ankiHost.backupTime`) | HDD (`/mnt/data/backups/anki-host/<instance>`, 인스턴스별 일일 `.colpkg`만; `backup.enable = false`인 검증용 `lab`은 제외. 복구점 `restore-points/`의 보존 계약은 [Anki 호스트 README](../../../modules/nixos/programs/anki-host/README.md)의 복구점 절) |
...
모바일 SSH 환경에서 Immich를 활용하여 AI 에이전트(Claude Code · Codex CLI 등)에 이미지 전달. 상세: [references/mobile-ssh-image.md](references/mobile-ssh-image.md)
