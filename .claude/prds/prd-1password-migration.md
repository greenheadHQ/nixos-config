...
- SC-4: SSH key 3개 (`mac-ssh` / `mobile-ssh`(iPhone·iPad 공유) / `emergency-fallback`)가 MiniPC `authorized_keys`에 등록되어 있다. iPhone/iPad는 Termius 계정 동기화로 디바이스별 키 격리가 불가하여 단일 `mobile-ssh` 공유 키를 쓴다(#866). 1Password Automation vault `ssh` tag에는 `mac-ssh`/`emergency-ssh`가 inventory된다(mobile은 Termius keychain 동기화 보유).
