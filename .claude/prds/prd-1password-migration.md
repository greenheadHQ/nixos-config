...
- G-4: SSH key·passkey·passwords의 디바이스별 inventory를 1Password에서 한눈에 본다 (SSH 키는 전용 ssh vault, 나머지 토큰은 Automation — #872 후속 P4로 분리).
...
- SC-2: gh PAT가 1Password Automation vault에 보관되고, `gh` 명령이 Mac은 대화형에서 1Password Shell Plugin alias(biometric)로 + 무인 에이전트(`c`/`codex` 런처)·다중 세션에서는 Mac 전용 SA token이 github-pat을 per-user temp 캐시에 무인 발급(#872, Touch ID 0회)하는 경로로, MiniPC는 opnix가 materialize한 github-pat을 주입하는 GH_TOKEN wrapper(headless)로 자동 인증된다. GitHub git transport(push/pull/fetch)는 Mac에서 `git_protocol=https` + `url."https://github.com/".insteadOf`로 gh credential helper(PAT) 경로를 거치며(SSH 키 미사용), MiniPC는 기존 SSH 경로를 유지한다 (#853, `pkgs.stdenv.isDarwin` 가드). 구 PAT는 GitHub 측에서 revoked 상태이고 `~/.config/gh/hosts.yml`에 평문 oauth_token이 없다.
- SC-3: Mac SSH 호출이 1Password SSH agent로 인증된다 (Touch ID popup). 1Password 장애 시 emergency ed25519 key로 fallback 가능하다 (실측 통과).
- SC-4: SSH key 3개 (`mac-ssh` / `mobile-ssh`(iPhone·iPad 공유) / `emergency-fallback`)가 MiniPC `authorized_keys`에 등록되어 있다. iPhone/iPad는 Termius 계정 동기화로 디바이스별 키 격리가 불가하여 단일 `mobile-ssh` 공유 키를 쓴다(#866). 1Password SSH 전용 vault(`constants.onePassword.vaults.ssh`)에 `mac-ssh`/`emergency-ssh`가 보관된다 — Automation에서 분리해 SA token blast radius를 github-pat 한정으로 격리한다(#872 후속 P4; mobile은 Termius keychain 동기화 보유).
...
- Expected outcome: iPhone Termius가 보유한 `mobile-ssh` private key(iPad와 Termius 동기화로 공유)로 인증. Mac과는 별개 키. MiniPC `authorized_keys`에 등록되어 있어 동작. 1Password가 down되어도 영향 없음.
...
- Actor: 사용자
- Trigger: iPad 분실
- Expected outcome: iPhone/iPad는 `mobile-ssh` 공유 키이므로 한 기기 분실 시 개별 revoke가 불가능하다 — `libraries/constants.nix`의 `constants.sshDeviceKeys.mobile`을 새 키로 rotate → `nrs minipc`로 MiniPC `authorized_keys` 교체 → 남은 기기에 새 mobile 키 재배포(Termius 동기화). 추가로 분실 기기를 Termius 계정에서 강제 로그아웃/디바이스 해제한다(동기화된 다른 자격증명 보호). Mac(`mac-ssh`)·`emergency` 영향 0.
...
- FR-10: SSH 키 3개 — `mac-ssh`(1Password agent-managed), `mobile-ssh`(iPhone·iPad 공유, Termius keychain device-local + 계정 동기화), `emergency-fallback`. iPhone/iPad 디바이스별 분리 키는 Termius 계정 동기화가 host의 key 지정과 표준 키 private 본체를 기기 간 복제해 격리가 불가하므로 단일 mobile 공유 키로 통합한다(#866; 진짜 디바이스 격리가 필요하면 Termius 동기화 OFF 또는 SSH ID passkey가 전제). 1Password SSH 전용 vault에 `mac-ssh`/`emergency` backup + revocation(공유 키는 rotate 기반) 절차 박제 (Automation에서 분리, SA token blast radius 격리 — #872 후속 P4). MiniPC authorized key source는 `libraries/constants.nix`의 `constants.sshDeviceKeys`와 이를 소비하는 `hosts/greenhead-minipc/default.nix`이다.
...
- A-2: iPhone/iPad SSH는 Termius 앱이 키를 보유한다 (1Password SSH agent의 iOS system-level socket 미지원). 단 Termius 계정 동기화가 키 private 본체와 host의 key 지정을 기기 간 복제하므로 "디바이스 로컬 격리"는 성립하지 않으며, iPhone/iPad는 단일 `mobile-ssh` 공유 키를 쓴다(#866 baseline 진단·웹조사로 확정). 디바이스별 격리가 필요하면 Termius 동기화 OFF 또는 SSH ID(passkey, device-bound)가 전제.
...
- iPhone/iPad는 Termius 계정 동기화로 `mobile-ssh` 단일 키를 공유한다(A-2). 디바이스별 격리/개별 revoke가 성립하지 않으므로, 한 기기 분실 시 `constants.sshDeviceKeys.mobile`을 rotate(`nrs minipc`로 교체)하고 분실 기기를 Termius 계정에서 해제한다. 진짜 디바이스 격리가 보안 요구로 격상되면 Termius 동기화 OFF 또는 SSH ID(passkey)로 전환한다.
...
- 2026-05-31: Phase 2a mobile SSH follow-up 해소(#866). hardening(`KbdInteractiveAuthentication = false`)은 #867로 선행 배포. iphone-ssh/ipad-ssh 신규 rotation(#868) 후, **Termius 계정 동기화가 host의 key 지정과 표준 키 private 본체를 기기 간 복제해 iPhone/iPad 디바이스별 격리가 구조적으로 불가**함을 웹조사 + MiniPC journal 실측으로 확정. SSH ID(passkey, device-bound)는 격리 가능(MiniPC OpenSSH 10.3p1이 `sk-ssh-ed25519@openssh.com` 지원 확인)하나 설정 복잡도로 보류. → iPhone/iPad를 단일 `mobile-ssh` 공유 키로 통합: `constants.sshDeviceKeys`의 `iphone`/`ipad`를 `mobile`(직전 iphone-ssh 키 재명명)로, MiniPC authorizedKeys를 3개(`mac-ssh`/`mobile-ssh`/`emergency`)로 축소. SC-4·FR-10·Scenario 4·5·A-2·Risks를 디바이스별 4키 → 공유 3키로 개정. 분실 대응은 mobile 키 rotate(`nrs`) + Termius 계정 디바이스 해제 모델.
- 2026-05-31: Phase 2b 무인 에이전트 gh 후속 — Mac 무인 gh 인증을 방식 B(SA token → github-pat 캐시)로 신설(#872, epic Phase 2b 후속). 직전 #848/#871(per-session op read, Touch ID 1회)이 무인·다중 에이전트(동시 다수)에 부적합 → Mac 전용 SA token(`nixos-automation-mac`, Automation read-only, 90일 만료)을 신규 발급해 `opnix-service-account-token-mac.age`(recipient=개인 Mac 키 단독, `personal` hostType 가드)로 agenix 배포. 로그인 셸 `_gh_pat()`이 SA token으로 github-pat을 per-user temp(`getconf DARWIN_USER_TEMP_DIR`, 0700 디렉토리/0600 파일, 재부팅 휘발) 캐시에 무인 발급(Touch ID 0회) → `gh`/`c`/`codex`가 GH_TOKEN으로 사용. SA token은 op 프로세스 env로만 전달(셸 env 미상주), 캐시엔 github-pat만. 이로써 운영 모델이 "전역 공유 SA 1개·Mac 미사용"(2026-05-24 항목)에서 "Mac 전용 격리 SA 추가"로 갱신됨(blast radius 격리). `work` hostType은 SA 미배포 → biometric/기존 경로 graceful. E2E 실측: 무인 `gh api user`=greenheadHQ Touch ID 0회(첫 발급 2~5s·캐시 hit 0.4s), 캐시 0600·내용은 github-pat(SA token 아님), SA token env 노출 0, corrupt 캐시 자가회복. 멀티에이전트 검토로 confirmed 4건 반영 — 캐시 world-writable `/tmp` 폴백을 per-user 경로로(타 로컬 사용자 캐시 선점 차단), 12h mtime TTL + github-pat prefix 검증(stale/truncated 토큰 silent 사용 방지), `secrets.nix` 주석 정정. 후속: SSH 키 vault 분리(SA blast radius를 github-pat 한정으로 축소)는 별도 트랙.
- 2026-05-31: #872 후속 P4 — SSH 키 vault 분리. `mac-ssh`/`emergency-ssh`를 Automation → SSH 전용 vault(`constants.onePassword.vaults.ssh` 신설)로 이동하고 `agent.toml` 노출 vault를 ssh로 변경. 과도기에 Automation+ssh 두 vault를 병행 노출해 키 이동 중 SSH lockout을 막고, 검증 후 Automation 라인을 제거했다. 목적: Mac 전용 SA token(Automation read-only)이 SSH 개인키에 접근하지 못하게 해 SA blast radius를 github-pat 한정으로 축소. 실측: `ssh minipc` 정상(SSH vault의 mac-ssh agent 인증, Touch ID), `ssh-add -l`에 mac-ssh/emergency-ssh 노출 유지, SA token으로 `op read` 시 SSH vault(`isn't a vault in this account`)·Automation의 mac-ssh(active·deleted) 모두 차단, SA의 Automation active 목록은 github-pat+SA item뿐. `emergency_ed25519`(파일, IdentityAgent=none)는 불변. SC-4·FR-10·G-4 갱신. 잔여: `op item move` 원본이 Automation Recently Deleted에 30일 잔존하나 SA 미접근 확인 → 위생상 GUI 영구 삭제 권장.
- 2026-05-31: #872 후속 — Mac SA token 90일 rotation 만료 알림 신설. weekly launchd user agent(`opnix-rotate-mac`, `modules/darwin/programs/opnix-rotate.nix`)가 평문 expiry record(`secrets/opnix-service-account-expiry-mac.txt`, op CLI 만료 조회 미지원 대체)를 읽어 만료 14일 이하면 Pushover(`pushover-share`)로 rotation 안내를 보낸다. MiniPC systemd `opnix-rotate`의 launchd 이식 — GNU `date -d`(writeShellApplication `coreutils`), `personal` hostType 가드(work 호스트는 Mac SA 미배포라 알림 대상 아님). 실측: launchd agent 등록 + 수동 실행 `89 days left` silent(>14일 미발송), `darwin system` build로 shellcheck 통과. SA 재발급 시 expiry record를 수동 갱신(SSOT). 만료일 초기값 2026-08-29(콘솔 실제 만료일로 정정 필요 시 record만 수정).
- 2026-06-01: Phase 5(skill 리팩토링, #878 merged — master table가 stale했어 함께 정정) + Phase 6(Vaultwarden EOL) 완료. Vaultwarden 서비스/모듈/스킬/agenix atomic 삭제 + cross-ref cleanup(managing-minipc/running-containers/managing-secrets) + 6개월 purge reminder(`pushover-purge-reminder.nix`, 2026-12-01) 신설. MiniPC 컨테이너/타이머 중지 + 백업 중립 archive(`password-manager-2026-06-01` + PURGE 마커) + nrs 배포(16 vaultwarden units 제거, reminder timer 등록 확인). 적대적 DA 리뷰(8 도메인) + 병렬 전수조사(6 bundle)가 acceptance gate 결함(`rg -w` word boundary가 한글 조사형 누락 + `--hidden` 부재 → 진짜 게이트 `rg -i --hidden`)으로 놓칠 뻔한 5곳을 추가 보정해 잔존 0건 달성. nix flake check + eval-tests + smoke-test 통과. Phase 5 잔여 후속 ①(constants 주석 #874) + ②(managing-secrets eval negative 가드 2쌍) 동봉. epic #780 마지막 phase 종료(Phase 4 GUI 완료·PR 대기만 잔여). PR Closes #780/#879.
