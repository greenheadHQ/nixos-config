...
- **op CLI 멀티 계정**: op CLI에 개인(my.1password.com) + 회사(example) 계정이 함께 로그인되면 `op read`/`op item get` 모두 account 모호("multiple accounts") → `--account`(개인 계정 my.1password.com)로 고정. constants.onePassword.account에 박제. MiniPC(Phase 3)는 OP_SERVICE_ACCOUNT_TOKEN이 account를 결정하므로 호환성 별도 확인.
