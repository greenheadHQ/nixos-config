...
- 정리: `wt cleanup --auto` (MERGED 자동) 또는 `wt cleanup <name>...` (이름 지정). `--yes`는 "위험을 인지하고 강제 삭제"를 뜻한다 — dirty/unpushed 확인을 우회할 뿐 아니라, MERGED 무확인 삭제에 붙는 보호(비강제 제거·근거 재확인·ref CAS)도 함께 해제한다. **정리 대상 worktree 밖(저장소 루트)에서 실행한다** — 자기 자신은 삭제 시 셸의 cwd가 사라지므로 항상 제외된다. `wt`는 제외 사실을 알리고, 그 worktree가 실제 정리 대상(PR MERGED)이거나 이름으로 직접 지정된 경우에는 저장소 루트 재실행 명령도 함께 안내한다.
