...
- 목록: `wt ls --json` (name/branch/path/pr/dirty/unpushed/current/committedAt/age 구조화 출력, jq 파싱용). `unpushed`는 "정리하면 잃을 커밋이 있는가"를 뜻한다 — squash merge로 원격 브랜치가 사라져 upstream이 없어도, PR이 MERGED이고 그 판정 근거인 OID가 현재 HEAD와 일치하는 동안에는 `false`다 (`modules/shared/scripts/lib/wt/git-state.sh`의 `_wt_has_unpushed_risk`). `pr`은 조회 시점 스냅샷이라, 조회 뒤 새 커밋이 생기면 `pr: "MERGED"`와 `unpushed: true`가 함께 나올 수 있다 — 근거가 낡았다는 정상 신호이지 결함이 아니다.
