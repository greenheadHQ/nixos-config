...
- 목록: `wt ls --json` (name/branch/path/pr/dirty/unpushed/current/committedAt/age/locked/broken 구조화 출력, jq 파싱용). 목록은 디렉토리 스캔이 아니라 **git 등록(`git worktree list`) 기준**이라 depth 2 이상 경로(`.claude/worktrees/feat/x`)와 등록만 남고 디렉토리가 사라진 항목도 나오며, git이 모르는 잔재 디렉토리는 합집합으로 함께 나온다. `name`은 `.claude/worktrees` 기준 상대 경로다 — depth 2 이상 항목은 `feat/x`로 나오며, 그 값이 `wt cleanup`에 그대로 넘기는 이름이다. `broken: true`는 그 등록의 worktree를 읽을 수 없다는 뜻이다(디렉토리 소실·gitdir 무효) — branch는 등록에 기록된 값이고 committedAt/dirty/unpushed는 의미가 없다. `locked: true`는 `git worktree lock`된 항목으로, cleanup은 `--auto`·이름 지정·`--yes` 어느 경로에서도 이를 정리하지 않는다. `unpushed`는 "정리하면 잃을 커밋이 있는가"를 뜻한다 — squash merge로 원격 브랜치가 사라져 upstream이 없어도, PR이 MERGED이고 그 판정 근거인 OID가 현재 HEAD와 일치하는 동안에는 `false`다 (`modules/shared/scripts/lib/wt/git-state.sh`의 `_wt_has_unpushed_risk`). `pr`은 조회 시점 스냅샷이라, 조회 뒤 새 커밋이 생기면 `pr: "MERGED"`와 `unpushed: true`가 함께 나올 수 있다 — 근거가 낡았다는 정상 신호이지 결함이 아니다.
