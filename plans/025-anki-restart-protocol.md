...
- 노트 내용 수정/분할 (처리 절차 미정 — plan 027은 REJECTED), 애드온/문서 정리 (plan 026).
...
3. **비대 노트를 만나면**: 그 자리에서 고치지 않는다. Again/Hard로 평가만
   하고 **파란 깃발(Flag 1)** 을 붙인 뒤 넘어간다. (복습 중 편집은 세션을
   무한정 늘리는 함정.) 깃발은 Anki 프로필 안에 표시로만 유지한다 — 분할
   처리 절차는 **미정**이다 (담당 예정이던 plan 027은 2026-09-05 REJECTED).
   깃발이 쌓여 복습이 실제로 막히면 그때 절차를 별도로 정한다.
...
- 규칙 3(깃발)이 만드는 큐에는 현재 지정된 소비자가 없다 — 담당 예정이던
  plan 027이 2026-09-05 REJECTED됐고 대체 절차는 미정이다. 큐는 Anki 프로필
  안의 깃발로만 유지되며, 복습이 돌기 시작한 뒤 실제로 병목이 되면 그때
  절차를 정한다 (그 시점에 별도 plan/이슈로 승격).
